...
**Task** — 构建 AI Agent，用自然语言驱动自动化 SQL 对账

**Action**
- 基于 ReAct 范式设计 Agent 推理循环，5 步完成对账工作流
- 自研 Expandable Tool 模式：一个父类展开多个子工具，统一注册与发现
- SQL 执行前先 EXPLAIN 校验 + DDL/DML 安全拦截，杜绝幻觉
- 跨列名比对算法：按列位置 INDEX JOIN，支持异构表结构差异检测

**Result** — 一次典型对账（2 表、50+ 行）5-7 步自动完成，差异识别率 100%
